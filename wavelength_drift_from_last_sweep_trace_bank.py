...
def main():
    parser = argparse.ArgumentParser(
        description="Estimate post-sweep wavelength drift by matching traces to the experimental last-sweep trace bank."
    )
    ...
    parser.add_argument("--sweep-index", type=int, default=-1, help="Sweep interval index to use as reference; -1 means last full interval")
    parser.add_argument("--post-start-s", type=float, default=None, help="Post-sweep analysis start; default is selected sweep end")
    parser.add_argument("--post-end-s", type=float, default=5.5, help="Post-sweep analysis end time")
    parser.add_argument("--initial-window-traces", type=int, default=50, help="Initial same-parity traces used as drift zero")
    parser.add_argument("--tracking-step-sigma-pm", type=float, default=0.03, help="Continuity scale for reference-bank tracking")
    parser.add_argument("--rolling-window", type=int, default=128, help="Rolling window for plotted curves")
    parser.add_argument("--block-rows", type=int, default=512, help="Rows per correlation block")
    ...
    ax1.plot(merged_time_abs_s, merged_lambda_pm, ".", color="#B0B0B0", markersize=2.0, alpha=0.45, label="Merged")
    ax1.plot(parity_results["even"]["time_abs_s"], parity_results["even"]["lambda_pm"], ".", color="#1F77B4", markersize=1.8, alpha=0.55, label="Even")
    ax1.plot(parity_results["odd"]["time_abs_s"], parity_results["odd"]["lambda_pm"], ".", color="#FF7F0E", markersize=1.8, alpha=0.55, label="Odd")
    ax1.axvline(sweep_end_s, color="#D62728", linewidth=1.0, alpha=0.8, label="Last sweep end")
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Best coordinate inside last sweep (pm)")
    ax1.set_title("Wavelength coordinate after laser modulation is off")
    ax1.grid(alpha=0.25)
    ax1.legend(loc="best")
    ...
    ax2.plot(merged_time_abs_s, merged_drift_pm, ".", color="#808080", markersize=2.0, alpha=0.45, label="Merged raw")
    ...
    ax2.set_ylabel("Drift from initial post-sweep level (pm)")
    ax2.set_title("Wavelength drift from trace-bank matching")
    ax2.grid(alpha=0.25)
    ax2.legend(loc="best")
    ...
    ax3.plot(merged_time_abs_s, merged_fit_corr, ".", color="#4C78A8", markersize=2.0, alpha=0.55, label="Raw")
    ax3.plot(merged_time_abs_s, merged_fit_corr_rolling, color="#111111", linewidth=1.6, label=f"Rolling ({args.rolling_window})")
    ax3.axvline(sweep_end_s, color="#D62728", linewidth=1.0, alpha=0.8, label="Last sweep end")
    ax3.set_xlabel("Time (s)")
    ax3.set_ylabel("Best correlation")
    ax3.set_title("Trace-bank fit quality")
    ax3.grid(alpha=0.25)
    ax3.legend(loc="best")
