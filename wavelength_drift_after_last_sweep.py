...
def main():
    parser = argparse.ArgumentParser(
        description="Оценить дрейф длины волны в течение секунды после последнего свипа по восстановленной гармонической модели последнего свипа."
    )
    ...
    parser.add_argument("--model-mat", default=None, help="MAT-файл, созданный solve_complex_amplitudes_from_harmonics.py")
    ...
    parser.add_argument("--rolling-window", type=int, default=128, help="Окно rolling average для графиков дрейфа")
    parser.add_argument("--min-period-s", type=float, default=0.03, help="Минимальный период свипа для детектора сбросов")
    parser.add_argument("--max-period-s", type=float, default=0.2, help="Максимальный период свипа для детектора сбросов")
    ...
    parser.add_argument("--max-reset-time-s", type=float, default=4.6, help="Игнорировать сбросы после этого времени")
    parser.add_argument("--reset-detection-end-time-s", type=float, default=4.6, help="Использовать только трассы до этого времени при поиске последнего свипа")
    parser.add_argument("--post-duration-s", type=float, default=1.0, help="Длительность анализа после последнего свипа")
    parser.add_argument("--lambda-grid-min-pm", type=float, default=-0.5, help="Минимальный fitted-сдвиг длины волны в pm")
    parser.add_argument("--lambda-grid-max-pm", type=float, default=4.0, help="Максимальный fitted-сдвиг длины волны в pm")
    parser.add_argument("--lambda-grid-step-pm", type=float, default=0.002, help="Шаг fitted-сетки длины волны в pm")
    parser.add_argument("--tracking-step-sigma-pm", type=float, default=0.01, help="Масштаб штрафа непрерывности ветви в pm на шаг трассы")
    parser.add_argument("--tracking-initial-window-traces", type=int, default=50, help="Сколько первых post-sweep трасс использовать для выбора начальной ветви")
    parser.add_argument("--cleanup-dataset-outputs", action="store_true", help="Удалить старые результаты этого датасета перед сохранением новых")
    ...
    ax1.plot(merged_time_rel_s, merged_lambda_pm, ".", color="#C7C7C7", markersize=2.0, alpha=0.30, label="Объединённые raw")
    ax1.plot(parity_results["even"]["time_rel_s"], parity_results["even"]["lambda_pm"], ".", color="#1F77B4", markersize=1.8, alpha=0.55, label="Чётные")
    ax1.plot(parity_results["odd"]["time_rel_s"], parity_results["odd"]["lambda_pm"], ".", color="#FF7F0E", markersize=1.8, alpha=0.55, label="Нечётные")
    ax1.set_xlabel("Время после последнего свипа (s)")
    ax1.set_ylabel("Подобранная координата длины волны (pm)")
    ax1.set_title("Оценка длины волны после последнего свипа с неоднозначностью ветви чётности")
    ax1.grid(alpha=0.25)
    ax1.legend(loc="best")
    ...
    ax2.plot(merged_time_rel_s, merged_drift_pm, ".", color="#7F7F7F", markersize=2.5, alpha=0.55, label="Объединённые raw")
    ax2.plot(merged_time_rel_s, merged_drift_pm_rolling, color="#111111", linewidth=1.6, label=f"Rolling ({args.rolling_window})")
    ax2.set_xlabel("Время после последнего свипа (s)")
    ax2.set_ylabel("Дрейф относительно первой post-sweep трассы (pm)")
    ax2.set_title("Дрейф длины волны в течение секунды после последнего свипа")
    ax2.grid(alpha=0.25)
    ax2.legend(loc="best")
    ...
    ax3.plot(merged_time_rel_s, merged_fit_corr, ".", color="#4C78A8", markersize=2.5, alpha=0.55, label="Сырые точки")
    ax3.plot(merged_time_rel_s, merged_fit_corr_rolling, color="#111111", linewidth=1.6, label=f"Rolling ({args.rolling_window})")
    ax3.set_xlabel("Время после последнего свипа (s)")
    ax3.set_ylabel("Лучшая корреляция модели")
    ax3.set_title("Качество fit-а для оценки длины волны после свипа")
    ax3.grid(alpha=0.25)
    ax3.legend(loc="best")
